...
export class PassportAuthentication {
  ...
  private setupWebRoutes(router: Router): void {
    ...
    router.get("/dashboard/apps", limiter, this._cookieSessionMiddleware, async (req: Request, res: Response) => {
    ...
    router.get("/dashboard/tokens", limiter, this._cookieSessionMiddleware, async (req: Request, res: Response) => {
      const accessKey: string = req.session["accessKey"];
      if (!accessKey) return res.redirect("/auth/login");
    ...
    router.get("/dashboard/apps/:appName", limiter, this._cookieSessionMiddleware, async (req: Request, res: Response) => {
      const accessKey: string = req.session["accessKey"];
      if (!accessKey) return res.redirect("/auth/login");
