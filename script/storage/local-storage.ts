...
export class LocalStorage implements storage.Storage {
  ...
  constructor() {
    this.storageDir = process.env.STORAGE_DIR || "/mnt/extra_storage";
    this.dbPath = path.join(this.storageDir, "database.json");
    this.blobsDir = path.join(this.storageDir, "blobs");
  ...
  private loadStateSync(): void {
    if (fs.existsSync(this.dbPath)) {
      try {
        ...
        LocalStorage.NextIdNumber = obj.NextIdNumber || 0;
        ...
        console.error("Failed to load state from", this.dbPath, err);
      }
    }
  }
  ...
  private saveStateAsync(): void {
    const obj = {
      NextIdNumber: LocalStorage.NextIdNumber,
      accounts: this.accounts,
      apps: this.apps,
      deployments: this.deployments,
      deploymentKeys: this.deploymentKeys,
    ...
    // Write to a temp file then rename it to ensure atomic writes
    const tempPath = this.dbPath + ".tmp";
    fs.writeFile(tempPath, str, (err) => {
      if (!err) {
        fs.rename(tempPath, this.dbPath, (renameErr) => {
          if (renameErr) console.error("Error saving state:", renameErr);
        });
      } else {
        console.error("Error writing temp state:", err);
      }
    });
  }
  ...
  public addAccount(account: storage.Account): Promise<string> {
    ...
    if (this.accounts[account.id] || this.accountToAppsMap[account.id] || this.emailToAccountMap[email]) {
      return LocalStorage.getRejectedPromise(storage.ErrorCode.AlreadyExists);
    }
  ...
  public getAccount(accountId: string): Promise<storage.Account> {
    if (!this.accounts[accountId]) {
      return LocalStorage.getRejectedPromise(storage.ErrorCode.NotFound);
    }
  ...
  public getAccountByEmail(email: string): Promise<storage.Account> {
    for (const id in this.accounts) {
      if (this.accounts[id].email.toLowerCase() === email.toLowerCase()) {
  ...
  public getAccountIdFromAccessKey(accessKey: string): Promise<string> {
    ...
    if (new Date().getTime() >= this.accessKeyNameToAccountIdMap[accessKey].expires) {
      return LocalStorage.getRejectedPromise(storage.ErrorCode.Expired, "The access key has expired.");
    }
  ...
  public transferApp(accountId: string, appId: string, email: string): Promise<void> {
    if (isPrototypePollutionKey(email)) {
      return LocalStorage.getRejectedPromise(storage.ErrorCode.Invalid, "Invalid email parameter");
    }
    return this.getApp(accountId, appId).then((app: storage.App) => {
      ...
      if (!targetOwnerAccountId) {
        return LocalStorage.getRejectedPromise(storage.ErrorCode.NotFound, LocalStorage.CollaboratorNotFound);
      }
  ...
  public removeCollaborator(accountId: string, appId: string, email: string): Promise<void> {
    return this.getApp(accountId, appId).then((app: storage.App) => {
      ...
      return this.updateApp(accountId, app, false);
    });
  }
  ...
  public updatePackageHistory(accountId: string, appId: string, deploymentId: string, history: storage.Package[]): Promise<void> {
    if (!history || !history.length) {
      return LocalStorage.getRejectedPromise(storage.ErrorCode.Invalid, "Cannot clear package history from an update operation");
    }
  ...
  public addBlob(blobId: string, readableStream: stream.Readable, streamLength: number): Promise<string> {
    return q.Promise<string>((resolve, reject) => {
      const filePath = path.join(this.blobsDir, blobId);
  ...
  public getBlobUrl(blobId: string): Promise<string> {
    ...
    return q(`${serverUrl}/blobs/${blobId}`);
  }
  ...
  private newId(): string {
    const id = "id_" + LocalStorage.NextIdNumber;
    LocalStorage.NextIdNumber += 1;
    return id;
  }
